...
plot_ora <- function(object, file = "", plot_type = "dotplot") {
  if (plot_type == "dotplot") {
    object %>% clusterProfiler::dotplot(
      x = "GeneRatio",
      font.size = 9,
      color = "p.adjust",
      showCategory = 20
  ...
  if (plot_type == "emapplot") {
    object %>% emapplot(
      showCategory = 20, color = "p.adjust", layout = "kk",
      vertex.label.cex = 1.2
    )
  }
  if (!nchar(file)) {
    file <- paste(plot_type, ".png", sep = "")
  }
  ggsave(file)
}
...
run_ora <- function(input, output) {
  ...
  padjust_method <- reactive({
    ...
    if (padjust_methods == "Bonferroni") {
      return("bonferroni")
    } else if (padjust_methods == "Holm") {
      return("holm")
    } else if (padjust_methods == "Hochberg") {
      return("hochberg")
    } else if (padjust_methods == "Hommel") {
      return("hommel")
    } else if (padjust_methods == "Benjamini & Hochberg") {
      return("BH")
    } else if (padjust_methods == "Benjamini & Yekutieli") {
      return("BY")
    } else if (padjust_methods == "None") {
      return("none")
    }
  })
  ...
  if (input$db_choice == "GO") {
    ...
    ora_result <- ora_kegg(
      genelist = genelist, organism = org_name,
      pvalueCutoff = pvalue_ora(),
      pAdjustMethod = padjust_method(),
      qvalueCutoff = qvalue_ora()
    )
  } else {
    warning("Error db choice")
  }
