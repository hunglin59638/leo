...
echo_prot_acc_demo <- function() {
  ids <- as.character(c("AAA48674.1", "XP_040547394.1", "XP_015154259.2",
...
prot_to_gene <- function(ids, fr_db="protein", target_db="gene") {
  ...
  names(ids_trim) <- ids
  all_links_sep <- list()
  for (chunk in split(sapply(ids_trim, function(x) {x[1]}), rep_len(1:100, length(ids_trim)))) {
    res <- entrez_link(db=target_db, dbfrom=fr_db, 
...
gi_to_protein_acc <- function(ids, reverse=F) {
  ...
  for  (chunk in split(ids, rep_len(1:100, length(ids)))) {
    tryCatch(
      fetch <- append(fetch, entrez_summary(db="protein", id=chunk, 
